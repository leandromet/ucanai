...
function SportPlanForm({ onSubmit }:{ onSubmit:(payload:ConsultationPayload, result:any)=>void }){
  ...
  return (
    <>
      ...
      <form onSubmit={(e)=>{e.preventDefault(); const payload:ConsultationPayload={activity,date,group_size:groupSize,skill_level:skill,duration_hours:duration,preferences:prefs,constraints:{budget_per_person:budget}}; const prompt = buildPrompt(payload); setPromptText(prompt); const result = mockGenerate(payload); setLocalResult(result); onSubmit(payload,result)}}>
        ...
        <label>Group size <input type="number" min={1} value={groupSize} onChange={e=>setGroupSize(Number(e.target.value))}/></label>
        <label>Skill level
        ...
        <label>Duration (hours) <input type="number" min={1} value={duration} onChange={e=>setDuration(Number(e.target.value))}/></label>
        <label>Budget per person <input type="number" min={0} value={budget} onChange={e=>setBudget(Number(e.target.value))}/></label>
